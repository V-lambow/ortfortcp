...
std::variant<bool, std::string> SAM2::initialize(std::vector<std::string> &onnx_paths, bool is_cuda)
{
    ...
    for (size_t index = 0; index < img_encoder_input_num; index++)
    {
        ...
        for (size_t j = 0; j < input_dims.size(); j++)
            node.dim.push_back(input_dims.at(j));
    ...
    for (size_t index = 0; index < img_encoder_output_num; index++)
    {
        ...
        for (size_t j = 0; j < output_dims.size(); j++)
            node.dim.push_back(output_dims.at(j));
...
std::variant<bool, std::string> SAM2::inference(cv::Mat &image)
{
    ...
    if (result_0.index() != 0)
        return std::get<std::string>(result_0);
    ...
    if (result_2.index() != 0)
        return std::get<std::string>(result_2);
    ...
    if (result_3.index() != 0)
        return std::get<std::string>(result_3);
...
std::variant<std::vector<Ort::Value>, std::string> SAM2::img_encoder_infer(std::vector<Ort::Value> &input_tensor)
{
    ...
    for (auto &node : this->img_encoder_input_nodes)
        input_names.push_back(node.name);
    for (auto &node : this->img_encoder_output_nodes)
...
{
    std::vector<const char *> input_names, output_names;
    for (auto &node : this->mem_attention_input_nodes)
        input_names.push_back(node.name);
...
{
    std::vector<const char *> input_names, output_names;
    for (auto &node : this->img_decoder_input_nodes)
        input_names.push_back(node.name);
    for (auto &node : this->img_decoder_output_nodes)
        output_names.push_back(node.name);
...
{
    std::vector<const char *> input_names, output_names;
    for (auto &node : this->mem_encoder_input_nodes)
        input_names.push_back(node.name);
    for (auto &node : this->mem_encoder_output_nodes)
        output_names.push_back(node.name);
    //***************************************************
...
void SAM2::postprocess(std::vector<Ort::Value> &output_tensors)
{
    ...
    // #pragma region 寻找与A中心距离最近的 bbox
    ...
    if (!contours.empty() && !contours[0].empty())
    {
        ...
        if (m.m00 != 0)
        {
            output_point = cv::Point2f(m.m10 / m.m00, m.m01 / m.m00);
            cv::circle(*ori_img, output_point, 3, cv::Scalar(50, 250, 200), 3);
        }
    }
}
...
// void SAM2::setFitParms(CenterSearch::LinesFitParams parms){
//     this->centerSearch_ptr.get()->setParam(parms);
// }