...
bool Yolov8PoseOnnx::OnnxBatchDetect(std::vector<cv::Mat>& srcImgs, std::vector<std::vector<OutputParams>>& output) {
	...
	int class_num = 2;
	int key_point_length = net_width - 4 - class_num;
	int key_point_num = 12; //_bodyKeyPoints.size(), shape[x, y, confidence]
	if (key_point_num * 3 != key_point_length) {
		std::cout << "Pose should be shape [x, y, confidence] with 12-points" << std::endl;
		return false;
	}
		...
		for (int r = 0; r < rows; ++r) {    
			...
			if (max_class_socre >= _classThreshold) {
				...
				for (int kpt = 0; kpt < key_point_length; kpt += 3) {
					...
					temp_kp.x = (pdata[6 + kpt]- params[img_index][2]) / params[img_index][0];
					temp_kp.y = (pdata[7 + kpt] - params[img_index][3]) / params[img_index][1];
					temp_kp.confidence = pdata[8 + kpt];
